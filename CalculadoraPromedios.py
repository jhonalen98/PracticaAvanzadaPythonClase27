...
# Funcion para contar las notas excluidas (TAREA)
def excluir_notas(notas):
    notas_excluidas=[]
    for nota in notas:
        if nota <= umbral:  
            notas_excluidas.append(nota)
    return len(notas_excluidas)
...
def main():
    ...
    notas_filtradas = filtrar_notas(notas)
    promedio = calcular_promedio(notas_filtradas)
    notas_excluidas = excluir_notas(notas)
    print(f"El promedio de las notas que están por encima del umbral ({umbral}) es: {promedio}, y el numero de notas excluidas es {notas_excluidas}")
